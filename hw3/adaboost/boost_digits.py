...
for iter in range(niter):
    ...
    # complete the adaBoost algorithm

    # Calculate weighted error with safe division

    e = np.sum(weights * (1 - correct)) / (np.sum(weights) + np.finfo(float).eps)


    alpha = 0.5 * np.log((1 - e) / (e + np.finfo(float).eps))

    # Update the weights
    weights = weights * np.exp(-alpha * (2 * correct - 1))

    # Normalize the weights
    weights /= (np.sum(weights) + np.finfo(float).eps)
